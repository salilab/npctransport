...
: public SingletonScore
{
 ...
 public:
  ...
  double evaluate_index(Model *m, const ParticleIndex &p,
...
#ifndef IMP_DOXYGEN
inline double HarmonicSpringSingletonSingleton::evaluate_index(
  ...
  ParticleIndex pi0(s.get_bonded_particle_0());
  ParticleIndex pi1(s.get_bonded_particle_1());
  ...
  double scoreDelta = 2 * 0.5 * k * dDelta * dDelta; // x2 because each particle applies force on the spring independently in this model of a relaxing spring
  ...
  if (IMP_LIKELY( da && delta_length > MIN_DISTANCE )) { // Profiling note on use of likely(): in BD simulations, the simulation bottleneck is when da is true, and the spring is likely out of equilibrium
    ...
    s.add_to_rest_length_derivative(fSpring); 
