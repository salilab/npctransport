...
inline double evaluate_one_site(
    ...
    algebra::Rotation3D itr0, algebra::Rotation3D itr1,
  ...
  if (da && distance > MIN_DISTANCE) {
    ...
    rb0.add_to_derivatives(itr0.get_rotated(duv), duv, local0,
                           tr0.get_rotation(), *da);
    rb1.add_to_derivatives(itr1.get_rotated(-duv), -duv, local1,
                           tr1.get_rotation(), *da);
  }
  return score;
}
...
inline double evaluate_one_site_2(
  ...
  if (da && distance2 > MIN_DISTANCE) {
    rb0.add_to_derivatives(itr0.get_rotated(deriv), deriv, local0,
                           tr0.get_rotation(), *da);
    rb1.add_to_derivatives(itr1.get_rotated(-deriv), -deriv, local1,
                           tr1.get_rotation(), *da);
  }
  return score;
}
