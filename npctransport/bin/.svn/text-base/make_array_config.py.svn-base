...
create_range(config.interaction_k, .2, 20, 10)
create_range(config.backbone_spring_k, .2, 20, 10)
config.time_step_factor.lower=1
create_range(config.nonspecific_range, 0, 5, 3, base=1)
create_range(config.range, 1, 5, 3, base=1)
#create_range(config.rest_length_factor, .5, 1, 10)
config.box.on_or_off.lower=1
config.box.size.lower=1000
...
create_range(fg.number, 10, 100, 10)
