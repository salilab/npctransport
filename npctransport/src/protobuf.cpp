      ...
      if (fd->type()== FieldDescriptor::TYPE_MESSAGE) {
        if (fd->is_repeated()) {
          ...
          for (int i=0; i< sz; ++i) {
            ...
            oss << fd->name() << i;
            show_ranges(oss.str(), &r->GetRepeatedMessage(*message, fd,
                                                          i));
          }
        } else {
...
void set_value(const Reflection *r, Message *m,
               const FieldDescriptor *fd, double v) {
  if (fd->type()== FieldDescriptor::TYPE_DOUBLE) {
    ...
    for (int i=0; i < d->field_count(); ++i) {
      ...
      if (fd->type()== FieldDescriptor::TYPE_MESSAGE) {
        if (fd->is_repeated()) {
          int sz= r->FieldSize(*message, fd);
          for (int i=0; i< sz; ++i) {
            ret+=get_ranges(fd->name(),
                            &r->GetRepeatedMessage(*message, fd,
                                                   i),
...
assign_ranges(std::string fname, std::string ofname, unsigned int work_unit,
              bool show_steps, std::uint64_t random_seed) {
  IMP_FUNCTION_LOG;
  std::fstream in(fname.c_str(), std::ios::in | std::ios::binary);
  if (!in) {
