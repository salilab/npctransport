...
def get_basic_config():
    ...
    config.time_step_factor.lower=1.0 / math.sqrt(fg_coarse_factor)
    #create_range(config.rest_length_factor, .5, 1, 10)
    config.time_step_wave_factor.lower=100 #### NOTE THIS ####
    config.excluded_volume_k.lower=1.0 / math.sqrt(fg_coarse_factor)
    ...
    config.angular_D_factor.lower=0.2 #lower to account for increased dynamic viscosity
...
kaps.k_z_bias_fraction.lower=z_bias_fraction
############### ACTIVE RANGE #############
create_range(kaps.interaction_k_factor, lb=0.01, ub=5, steps = 10, base=2)
##########################################
...
nonspecifics1.k_z_bias_fraction.lower=z_bias_fraction
...
nonspecifics2.k_z_bias_fraction.lower=z_bias_fraction
