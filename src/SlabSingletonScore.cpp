...
double SlabSingletonScore::evaluate_index(Model *m, const ParticleIndex pi,
  ...
  std::pair<double, Vector3D> dp = get_displacement_vector(d.get_coordinates());
  IMP_LOG(VERBOSE,
          "At point " << d.get_coordinates() << " have distance " << dp.first
                      << " and direction " << dp.second << std::endl);
  if (dp.first > d.get_radius()) return 0;
