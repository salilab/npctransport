...
create_serial_mover(const ParticlesTemp &ps, double scale_factor=1.0) {
...
void
rescale_move_size(core::MonteCarlo* mc, double scale_factor=1.0) {
...
void optimize_balls(const ParticlesTemp &ps,
  ...
  for (int i=0; i< 11; ++i) {
    ...
    double desired_accept_rate = 0.2; // acceptance rate for which to tune move size (per mover)
    for (int j=0; j< 5; ++j) {
        ...
        IMP_OMP_PRAGMA( single)
        {
          ...
          for(int k=0; k<n_external; k++){
           ...
           // std::cout << "Energy is " << e_mc << " at " << i <<
            ...
            // }
            // std::cout << "Rescaling move size and kt by " << move_rescale << std::endl;
            // rescale_move_size( mc, move_rescale);
            // kt /= move_rescale;
            double e_bd=local->optimize(n_bd_inner);
            std::cout << "Energy after bd is " << e_bd
                      << " at " << i
                      << ", " << j << "," << k << std::endl;
