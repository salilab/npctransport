...
def add_fg_based_on(config, mrc_filename, k, nfgs, nres, origin=None,
                    coarse_factor=fg_coarse_factor, really_add=True):
...
max_y=0
max_z=0
mean_loc=(add_fg_based_on(config, "MRCs/Nup57_16copies_chimera.mrc", k=16, nfgs=16, nres=240, really_add=False))
add_fg_based_on(config, "MRCs/Nup49_16copies.mrc", k=16, nfgs = 17, nres=240, origin=mean_loc, really_add=False)
add_fg_based_on(config, "MRCs/Nsp1_16copies_1.mrc", k=16, nfgs = 33, nres=600,  origin=mean_loc, really_add=False)
add_fg_based_on(config, "MRCs/Nsp1_16copies_2.mrc", k=16, nfgs = 33, nres=600, origin=mean_loc, really_add=False)
add_fg_based_on(config, "MRCs/Nup159_8copies.mrc", k=8, nfgs=23, nres=330, origin=mean_loc, really_add=False) # nfgs 20-25 = real number for Nup159, depending how you count double motifs and motif regions
add_fg_based_on(config, "MRCs/Nup116_8copies_chimera.mrc", k=8, nfgs=46, nres=720, origin=mean_loc, really_add=False)
add_fg_based_on(config, "MRCs/Nup42_8copies_chimera.mrc", k=8, nfgs=21, nres=370,  origin=mean_loc, really_add=False) # nfgs 21-27, depending on treratment of double motifs
add_fg_based_on(config, "MRCs/Nup100_8copies_chimera.mrc", k=8, nfgs=44, nres=600, origin=mean_loc, really_add=False)
add_fg_based_on(config, "MRCs/Nup145N_8copies_1_chimera.mrc", k=8, nfgs=11, nres=220, origin=mean_loc, really_add=False)
add_fg_based_on(config, "MRCs/Nup145N_8copies_2_chimera.mrc", k=8, nfgs=11, nres=220, origin=mean_loc, really_add=False)
# Nuclear:
add_fg_based_on(config, "MRCs/Nup1_8copies.mrc", k=8, nfgs=17, nres=675)#, origin=mean_loc, really_add=True)
###################### ACTIVE RANGE ##################
create_range(config.fgs[-1].interaction_k_factor, lb=1.0, ub=3.0, steps=2, base=1)
######################################################
#add_fg_based_on(config, "MRCs/Nup59_16copies.mrc", k=16, nfgs=5, origin=mean_loc, really_add=False) # contains also RRM Nup35-type domain for RNA binding (res 265-394 ; Uniprot), which supposedly overlaps some of the FGs
add_fg_based_on(config, "MRCs/Nup60_8copies.mrc", k=8, nfgs=10, nres=300, origin=mean_loc, really_add=False) # nup60 is supposed to tether Nup2 (depending on Gsp1p-GTP (Ran) switch, for which Nup2 has a binding site 583-720 ; Denning, Rexach et al. JCB 2001) ; Nup2 also interacts with cargo 35-50 and RNA (RRM Nup35-type domain) - from Uniprot)
#add_fg_based_on(config, "MRCs/Nup53_16copies_chimera.mrc", k=16, nfgs=4, origin=mean_loc, really_add=False) # contains also RRM Nup35-type domain for RNA binding (res 247-352 ; Uniprot), which supposedly overlaps some of the FGs ; and also a PSE1/Kap121 binding domain in a non-FG fashion, for which there might be a crystal structure (405-438 ; Uniprot)
