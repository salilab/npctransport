...
def get_basic_config():
    ...
    config.time_step_factor.lower=1.0
#    create_range(config.time_step_factor, lb=0.3, ub=0.6, steps=2, base=2)
    config.time_step_wave_factor.lower=1.0
#    create_range(config.time_step_wave_factor, lb=1, ub=3.0, steps=2, base=1)
...
def add_fg_based_on(config, mrc_filename, k, nfgs, nres,
                    k_fgkap_factor=1.0,
                    origin=None,
    ...
    ''' Read mrc_filename, cluster to k clusters, and create k
        ...
        @param k_fgkap_factor factor by which to increase interaction with kaps
                              relative to other fgs
...
#            = read_nups.cluster_MRC_file(mrc_filename, k)
    ...
    add_interactions_for_fg(type_name, k_fgkap * k_fgkap_factor)
...
max_y=0
max_z=0
mean_loc=(add_fg_based_on(config, "MRCs/Nup57_16copies_chimera.mrc", k=16, nfgs=16, nres=240))
add_fg_based_on(config, "MRCs/Nup49_16copies.mrc", k=16, nfgs = 17, nres=240, origin=mean_loc)
...
# Nuclear:
add_fg_based_on(config, "MRCs/Nup1_8copies.mrc", k=8, nfgs=17, nres=675, k_fgkap_factor=Nup1_kap_factor, origin=mean_loc)
...
###################### ACTIVE RANGE ##################
create_range(kaps.interaction_k_factor, lb=3.0, ub=11.0, steps = 5, base = 1)
######################################################
#create_range(kaps.interactions, lb=4, ub=12, steps = 3, base = 1)
...
nonspecifics= IMP.npctransport.add_float_type(config,
                                              ...
                                              radius=kaps_R, #-1,
                                              interactions=0)
nonspecifics.k_z_bias.lower=z_bias
nonspecifics.k_z_bias_fraction.lower=z_bias_frac
