...
#        bd.add_optimizer_state(sos)
        ...
        for x_ds1,estimated_score in [
                ...
                (2*radius+0.5*site_range,
                 -0.5*k_nonrot*site_range - 0.5*nonspec_k*nonspec_range),
...
#          ps.set_log_level(IMP.VERBOSE)
    ...
    def _test_one_sliding(self, site_range, site_k,
        ...
        sites0 = [IMP.algebra.Sphere3D(IMP.algebra.Vector3D(radius,0,0), 0.0)]
        sites1 = [IMP.algebra.Sphere3D(IMP.algebra.Vector3D(
                  -math.sqrt(1.0)*0.5*radius, math.sqrt(0.0)*0.5*radius, 0.0),
                  0.0)]
...
#        print("RMF:",rmf_fname)
        ...
        for rr,s,sigma0,sigma1 in [
            (1.50, -0.25*k_rot*site_range**2 - nonspec_k*nonspec_range,
             0.0, 0.0),
            (1.50, -0.5*0.25*k_rot*site_range**2 - nonspec_k*nonspec_range,
             math.acos(0.5+0.5*math.cos(sigma1_max_rad)), 0.0),
            (1.50, -0.5*0.25*k_rot*site_range**2 - nonspec_k*nonspec_range,
             0.0, math.acos(0.5+0.5*math.cos(sigma2_max_rad))),
            ...
            (1.50+0.5*site_range/radius,
             ...
             sigma1_max_rad/2.0, sigma2_max_rad/3.0)]:
            print("--\nrr*radius= ", rr*radius,
